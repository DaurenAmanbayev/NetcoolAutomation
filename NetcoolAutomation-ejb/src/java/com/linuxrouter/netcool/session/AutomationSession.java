...
public class AutomationSession {
    ...
    public BasicResponse getAllplugins() {
        BasicResponse response = new BasicResponse();
        response.setSuccess(true);
        response.setPayLoad(automationDao.getAllPlugins());
        return response;
    }
    ...
    public BasicResponse updateConnectionByName(String name, String user, String pass, String url, String enabled) {
        ...
        if (connection != null) {
           ...
           // }else{
            ...
            connection = new AutomationConnection(name);
            connection.setEnabled(enabled);
            connection.setUsername(user);
            connection.setJdbcUrl(url);
            connection.setPassword(pass);
            automationDao.saveData(connection);
        }
        return response;
    }
    ...
    public BasicResponse updateReaderByName(String readerName, String connectionName, String cronString, String enabled) {
        ...
        if (reader != null) {
            ...
            //Reader is null
            reader = new AutomationReader();
            reader.setConnectionName(connection);
            reader.setReaderName(readerName);
            reader.setCronInterval(cronString);
            reader.setEnabled(enabled);
            reader.setLogin(automationDao.getUserByLogin("admin"));
            automationDao.saveData(reader);
    ...
    public BasicResponse updateFilterByName(String filterName, String readerName, String filterSql, String enabled) {
        ...
        if (filter != null) {
            ...
            filter = new AutomationReaderFilter();
            filter.setFilterName(filterName);
            AutomationReader reader = automationDao.getReaderByName(readerName);
            filter.setReaderName(reader);
            filter.setPersistState("Y");
            filter.setEnabled(enabled);
            filter.setFilterSql(filterSql);
            filter.setStateChange(0);
            try {
                automationDao.saveData(filter);
                response.setSuccess(true);
            } catch (Exception ex) {
            }
        }
    ...
    public BasicResponse getPluginByName(String filterName) {
        BasicResponse response = new BasicResponse();
        AutomationPlugins plugin = automationDao.getPluginByName(filterName);
        if (plugin != null) {
            response.setSuccess(true);
            response.setPayLoad(plugin);
        }
        return response;
    }
}
